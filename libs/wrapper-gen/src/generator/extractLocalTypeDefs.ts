...
import { Appearance, AriaLive, Connotation, LayoutSize, Position, Role, Shape, Size } from '@vonage/vivid';
import { kebabToPascal } from './utils/casing';
import { TypeUnion } from './types';
import { getTypescriptDefinitionPath } from './vividPackage';

export const extractLocalTypeDefs = (componentName: string, modulePath: string): Record<string, TypeUnion> => {
  ...
  for (const [enumName, enumObj] of Object.entries(enums)) {
    ...
    for (const line of lines) {
      if (line.includes(`type ${typeName}`)) {
        ...
        const enumValuesReferenced = line.match(new RegExp(`${enumName}(Decorative)?\\.[A-Za-z]+`, 'g')) ?? [];

        const stringLiterals = enumValuesReferenced.map(enumValueReferenced => {
          ...
          if (enumValueReferenced === `ConnotationDecorative.Pacific`) return `'pacific'`;
        ...
        localTypeDefs[typeName] = stringLiterals.map(stringLit => ({ text: stringLit, vuePropType: 'String' }));
      }
    }
  }
