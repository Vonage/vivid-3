...
const escapeDescription = (text: string) => escapeMarkdown(text).replace(/\n/g, ' ');
...
function generateTableWithType(objects: ComponentDef['attributes'] | ComponentDef['events'], typeHeader = 'Type') {
  return markdownTable([
    ...
    ...objects.map(({ name, type, description }) => {
      ...
      return [
        `**${name}**`,
        resolvedType.length === 1 ? `\`${resolvedType[0].text}\`` : `*Enum*:<br/>${generateEnumType(resolvedType)}`,
...
function generateTable(slots: ComponentDef['slots']) {
  return markdownTable([
    ['Name', 'Description'],
    ...slots.map(({ name, description }) => [`**${name}**`, escapeDescription(description)]),
  ]);
}
...
function generateMethodsTable(methods: ComponentDef['methods']) {
  ...
  return markdownTable([
    ...
    ...methods.map(({ name, args, returnType, description }) => [
      `**${name}**`,
      `\`(${args.map(({ name: argName, type }) => `${argName}: ${getTypeString(type)}`)}) => ${getTypeString(
        returnType
      )}\``,
      escapeDescription(description),
    ]),
