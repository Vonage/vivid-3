...
test('should render rich text identically to the editor', async ({
	page,
}: {
	page: Page;
}) => {
		...
		template: `
			<div class="comparison-container">
				<div class="comparison-item">
					...
					<vwc-rich-text-editor style="width: 500px"></vwc-rich-text-editor>
				</div>
				<div class="comparison-item">
					<h3>rich-text-view</h3>
					<vwc-rich-text-view style="display: block; width: 500px; margin: 8px 16px;"></vwc-rich-text-view>
				</div>
			</div>
		`,
		setup: async () => {
			await page.evaluate(() => {
				...
				editor.instance = config.instantiateEditor({
					initialDocument: {
						type: 'doc',
						content: [
							{
				...
				const view = window.document.querySelector('vwc-rich-text-view')!;
				view.view = config.instantiateView(editor.instance.getDocument());
			});
		},
	});

	await takeScreenshot(page, 'rich-text-view');
});
