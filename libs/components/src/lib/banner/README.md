...
<template>
	<VBanner text="A banner with an action button">
		<template #action-items><VButton href="https://vonage.com" target="_blank" appearance="filled" connotation="accent" label="Learn More" /></template>
	</VBanner>
</template>
```
...
```vue preview full
<script setup lang="ts">
import { VBanner, VIcon } from '@vonage/vivid-vue';
</script>

<template>
	<VBanner text="Banner with icon inside icon slot" connotation="announcement">
		<template #icon>
			<VIcon label="Vivid News:">
...
```html preview full
<vwc-banner text="Banner with icon inside icon slot" connotation="announcement">
