...
import {
	type RteTextColorPickerConfig,
	RteTextColorPickerFeature,
} from './text-color-picker';

const { text, textColor: color, heading1: h1, paragraph: p } = docFactories;

const features = (config?: RteTextColorPickerConfig) => [
	new RteBase({ heading1: true, heading2: true }),
	new RteToolbarFeature(),
	new RteTextColorPickerFeature(config),
];
...
describe('RteTextColorFeature', () => {
	...
	describe('picker color', () => {
		...
		it('should be the default color when cursor has no mark', async () => {
			const rte = await setup(
				features({
					onBlocks: [{ node: 'paragraph', defaultColor: '#000000' }],
				}),
				[p('Hello')]
			);
		...
		it('should be the empty when selection in node that do not support color', async () => {
			const rte = await setup(
				features({
					onBlocks: [],
				}),
		...
		it('should be the default color of an empty block', async () => {
			const rte = await setup(
				features({
					onBlocks: [{ node: 'paragraph', defaultColor: '#000000' }],
				}),
	...
	describe('picking color', () => {
		it('should allow setting color by default', async () => {
		...
		it('should not apply the default color of a block when applying across blocks', async () => {
			const rte = await setup(
				features({
					onBlocks: [
						{ node: 'heading1', defaultColor: '#000000' },
						{ node: 'paragraph', defaultColor: '#ff0000' },
					],
				}),
				[h1('Heading'), p('Red Paragraph')]
			);
			rte.selectText('Hea[ding', 'Red Parag]raph');
			...
			expect(rte.docStr()).toMatchInlineSnapshot(
				`
				"
				heading1('Hea', <textColor[color="#ff0000"]>'[ding'),
				paragraph('Red Parag|]raph')
				"
			`
			);
		});
