...
onMounted(() => {
	gridElement = document.querySelector('.data-grid');
	if (gridElement) {
		gridElement.addEventListener('focusin', handleFocusIn);
		gridElement.addEventListener('focusout', handleFocusOut);
		gridElement.addEventListener('cell-focused', handleCellFocused);
	}
});
...
onUnmounted(() => {
	if (gridElement) {
		...
		gridElement.removeEventListener('cell-focused', handleCellFocused);
	}
});
</script>
...
<template>
	<VButton
		label="No Tabbing"
		@click="changeTabbing(true)"
		class="button"
		:appearance="noTabbing ? 'filled' : undefined"
	/>
	<VButton
		label="Tabbing"
		@click="changeTabbing(false)"
		class="button"
		:appearance="!noTabbing ? 'filled' : undefined"
	/>
	...
	<VDataGrid :no-tabbing="noTabbing" class="data-grid">
...
<template>
	...
	<VDataGrid
		:rows-data="data"
		:focus-row-index="rowIndex"
		:focus-column-index="colIndex"
	/>
</template>
```
...
<template>
	...
	/>
	<br />
	<div>Cell Clicked: <span id="click-result">{{ clickResult }}</span></div>
</template>
```
