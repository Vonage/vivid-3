...
import { isListboxOption, ListboxOption } from '../option/option.js';
...
export abstract class Listbox extends FoundationElement {
		...
		switch (direction) {
			case -1: {
				nextSelectableOption = this.options.reduceRight(
					(
						nextSelectableOption: ListboxOption | null,
						thisOption: ListboxOption,
						index: number
					) =>
	 ...
	 *
	 * @param _ - the previous list of selected options
	 ...
	 *
	 * @param _ - the previous list of slotted options
