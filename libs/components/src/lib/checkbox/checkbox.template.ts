...
> = (context) => {
  ...
  >
    <div class="control">
			${when(x => x.checked, html<Checkbox>`<${iconTag} class="icon" type="check-solid"></${iconTag}>`)}
			${when(x => x.indeterminate, html<Checkbox>`<${iconTag} class="icon" type="minus-solid"></${iconTag}>`)}
      ${() => focusTemplate}
    </div>
