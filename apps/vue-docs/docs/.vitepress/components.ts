...
export function getComponentsMenu(menu:  DefaultTheme.SidebarItem[]):  DefaultTheme.SidebarItem[] {
  return menu.map(({ text, link}) => {
    const componentName = path.parse(link).base;

    return { text, link, items: existsSync(path.join(ExamplesFolder, componentName)) ? [
